...
async function main() {
  ...
  const publisherMap = new Map(
    (await prisma.publisher.findMany({ select: { id: true, name: true } })).map((p) => [
      p.name,
      p.id,
    ]),
  );
  ...
  for (const { id: userId } of users) {
    ...
    for (const b of sample) {
      ...
      const currentPage =
        status === ReadingStatus.READ ? b.pages : faker.number.int({ min: 0, max: b.pages - 1 });
      const rating = status === ReadingStatus.READ ? faker.number.int({ min: 1, max: 5 }) : null;
