    ...
    >
      ...
      <SidebarHeader className="flex flex-row items-center gap-2 border-b p-4">
        <div className="relative pl-6">
          ...
          <div className="bg-primary absolute -bottom-1 -right-1 h-4 w-4 animate-pulse rounded-full" />
        </div>
        <h1 className="bg-clip-text text-2xl font-bold">Literalis</h1>
      </SidebarHeader>
        ...
        <SidebarMenu className="flex h-auto flex-col gap-2 p-4">
          {[
            ...
            <SidebarMenuItem className="min-h-auto" key={item.label}>
              <SidebarMenuButton
                className="hover:bg-muted/60 group min-h-14 rounded-lg py-2 transition-all duration-200"
                tooltip={item.label}
                style={{ animationDelay: `${index * 50}ms` }}
              >
                <Link
                  href={item.href}
                  className="hover:bg-primary/5 duration-400 flex w-full items-center gap-3 rounded-lg p-2 transition-all ease-in-out hover:scale-105 hover:text-lg"
                >
                  <div className={`rounded-lg p-1.5 transition-transform`}>
                    <item.icon size={28} className="text-current" />
                  </div>
                  <h2 className="text-foreground text-base font-semibold transition-colors">
                    {item.label}
                  </h2>
                  {item.badge && (
                    <div className="bg-destructive text-destructive-foreground ml-auto flex h-6 w-6 animate-pulse items-center justify-center rounded-full text-xs font-bold">
                      {item.badge}
                    </div>
                  )}
                </Link>
      ...
      <SidebarFooter className="border-border/50 border-t">
        <SidebarMenu className="flex flex-col gap-2 px-2 py-6">
          <SidebarMenuItem>
            <div className="hover:bg-muted/60 group rounded-lg transition-all duration-200">
              <ModeToggle className="w-full p-2" verbose />
            </div>
          </SidebarMenuItem>
          <SidebarMenuItem>
            <SidebarMenuButton
              className="hover:bg-destructive/10 hover:text-destructive group cursor-pointer rounded-lg py-6 transition-all duration-200"
              onClick={() => signOut()}
              tooltip="Sair"
            >
              <div className="bg-destructive/10 text-destructive rounded-lg p-1.5 transition-transform group-hover:scale-110">
